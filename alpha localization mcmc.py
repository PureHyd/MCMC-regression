...
for i in range(Ntry):
    ...
    initialParam = [np.random.exponential(scale=16), np.random.exponential(scale=16),\
                    np.random.exponential(scale=100)]
    Niter = int(1e5)
    dist = 1
    T = 1e8
    param = initialParam
    ...
    eta = (Ntry-i-1) / (i+1) * (tnow - t) + t
    print("{}\t{}\t{}".format(eta, i, cost))
    data.append((X_data, Y_data))
    paramstore.append(param)
    resid.append(cost)
paramstore = np.array(initialParam)
...
nn = 4 #  top 1/4
sortedParam = paramstore[resultOrd[:resultLen//nn]]
sortedCost = resid[resultOrd[:resultLen//nn]]

for param, cost in zip(sortedParam, sortedCost):
    plt.plot(xx, deltaG(param, xx))
    param = np.abs(param)
    print("N:  {:.8f}\tB_d:  {:.8e}\tB_dphi:  {:.8e}\tcost:  {:.8e}"\
          .format(max(param[:2]), min(param[:2]), param[2], cost))
